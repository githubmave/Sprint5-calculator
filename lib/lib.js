...
function drawBoard (boardNode) {
 ...
 //boardNode.style.width = 4 * 85 + 'px'
 boardNode.style.width = 478 + 'px'
...
function addListeners (boardNode) {
  for (var i = 0; i < boardNode.children.length; i++) {
    ...
    boardNode.children[i].addEventListener('click', getKeyNumber)
...
function getKeyNumber(evt){var
  ...
  if(cell.value>=0 ){
    ...
    //lib.formulorItems.push(cell.value);

   
  }
  else if(cell.value==-1||cell.value==-2||cell.value==-3||cell.value==-4){

         
        if(lib.formulorItems.length!=0&&lib.formulorItems[lib.formulorItems.length-1]>=0){
          //!(lib.formulorItems[lib.formulorItems.length-1]<0)
          lib.formulorItems.push(cell.value);
